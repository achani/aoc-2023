...
def get_prediction_future(values):
    if Counter(values).most_common()[0][1] == len(values):
        ...
        return values[-1] + get_prediction_future([(values[i+1] - values[i]) for i in range(len(values)-1)])

def get_prediction_past(values):
    if Counter(values).most_common()[0][1] == len(values):
        return values[0]
    else: 
        return values[0] - get_prediction_past([(values[i+1] - values[i]) for i in range(len(values)-1)])
