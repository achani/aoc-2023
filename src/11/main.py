...
def get_sum(spacing):
    ...
    for c in galaxy_comb:
        ...
        empty_rows_between = len(
            [row for row in empty_rows if min(r1, r2) < row < max(r1, r2)]
        )
        empty_cols_between = len(
            [col for col in empty_columns if (min(c1, c2) < col < max(c1, c2))]
        )
        sum_ += (
            abs(r2 - r1)
            + (empty_cols_between * (spacing - 1))
            + abs(c2 - c1)
            + (empty_rows_between * (spacing - 1))
        )
    return sum_
...
empty_columns = [
    ...
    if "#" not in [grid[x][i] for x, _ in enumerate(grid)]
]
galaxy_locations = [
    (i, j) for i, r in enumerate(grid) for j, c in enumerate(r) if c == "#"
]
